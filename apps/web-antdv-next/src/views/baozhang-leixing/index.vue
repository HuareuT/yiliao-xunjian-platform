...
import type { BaozhangLeixingRecord } from './data';
...
import {
  mockBaozhangLeixingRecords,
  useColumns,
  useGridFormSchema,
} from './data';
import FormDrawer from './modules/form.vue';

const records = ref<BaozhangLeixingRecord[]>([
  ...mockBaozhangLeixingRecords,
]);
...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<BaozhangLeixingRecord>,
});
...
function onEdit(row: BaozhangLeixingRecord) {
  editDrawerApi.setData(row).open();
}

function onDelete(row: BaozhangLeixingRecord) {
  Modal.confirm({
    content: `确认删除保障类型「${row.name}」吗？`,
    onOk() {
...
async function onStatusChange(newStatus: number, row: BaozhangLeixingRecord) {
...
function upsertRecord(row: BaozhangLeixingRecord) {
