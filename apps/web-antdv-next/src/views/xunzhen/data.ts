...
const careInfoMap: Record<string, XunzhenCareInfo> = {
  高血压随访: {
    ...
    treatments: [
      { name: '静养观察', remark: '减少剧烈运动，连续记录晨起和睡前血压' },
    ],
  },
  糖尿病复查: {
    medications: [
    ...
    treatments: [
      { name: '静养观察', remark: '记录空腹及餐后血糖，避免空腹运动' },
    ],
  },
  术后康复评估: {
    medications: [
  ...
  呼吸道感染: {
    ...
    treatments: [
      { name: '静养观察', remark: '保持室内通风，多饮水，观察呼吸频率' },
    ],
  },
  冠心病管理: {
    medications: [
    ...
    treatments: [
      { name: '静养观察', remark: '避免情绪激动和剧烈运动，记录胸闷胸痛情况' },
    ],
  },
  脑卒中康复: {
    medications: [
  ...
  妊娠期血糖管理: {
    ...
    treatments: [
      { name: '静养观察', remark: '记录饮食和血糖变化，异常时联系医生' },
    ],
  },
  慢阻肺随访: {
    medications: [
    ...
    treatments: [
      { name: '静养观察', remark: '避免冷空气刺激，观察喘息和活动耐量' },
    ],
  },
  贫血原因待查: {
    medications: [
...
function fuzzyMatchList(values: string[], keyword?: string) {
  const trimmedKeyword = keyword?.trim();
  return (
    !trimmedKeyword || values.some((value) => value.includes(trimmedKeyword))
  );
}
...
function getCareInfos(row?: XunzhenRecord) {
  return (
    row?.diagnosisType
      .map((diagnosisType) => careInfoMap[diagnosisType])
      .filter((item): item is XunzhenCareInfo => !!item) ?? []
  );
}

export function getMedicationNames(row?: XunzhenRecord) {
  return uniqueValues(
    getCareInfos(row).flatMap((careInfo) =>
      careInfo.medications.map((item) => item.name),
    ),
  );
}

export function getTreatmentNames(row?: XunzhenRecord) {
  return uniqueValues(
    getCareInfos(row).flatMap((careInfo) =>
      careInfo.treatments.map((item) => item.name),
    ),
  );
}

export function formatMultiValue(value?: string | string[]) {
...
export async function getMockXunzhenList(params: XunzhenQueryParams) {
  ...
  const filtered = mockXunzhenRecords.filter((item) => {
    return (
      ...
      fuzzyMatchList(item.diagnosisType, params.diagnosisType) &&
      fuzzyMatchList(getMedicationNames(item), params.medicationName) &&
      fuzzyMatchList(getTreatmentNames(item), params.treatmentName) &&
...
export function useColumns(): VxeTableGridColumns<XunzhenRecord> {
  const multiLineCellClassName = 'xunzhen-multi-line-cell';
  ...
  return [
    ...
    },
    {
      ...
      field: 'diagnosisType',
      formatter: ({ cellValue }) => formatMultiValue(cellValue),
      minWidth: 180,
      title: '诊断类型',
    },
    {
      className: multiLineCellClassName,
      field: 'medicationNames',
      formatter: ({ row }) => formatMultiValue(getMedicationNames(row)),
      minWidth: 220,
      title: '开药名称',
    },
    {
      ...
      formatter: ({ row }) => formatMultiValue(getTreatmentNames(row)),
      minWidth: 180,
      title: '处置方式',
    },
    {
...
export function useDescriptionItems(
  ...
  return [
    ...
    { label: '诊断类型', content: formatMultiValue(row?.diagnosisType) },
...
export function useMedicationDescriptionItems(
  ...
  const medications = getCareInfos(row).flatMap(
    (careInfo) => careInfo.medications,
  );
...
export function useTreatmentDescriptionItems(
  ...
  const treatments = getCareInfos(row).flatMap(
    (careInfo) => careInfo.treatments,
  );
